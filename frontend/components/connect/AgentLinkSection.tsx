...
export function AgentLinkSection() {
    ...
    <>
      <div className="agent-link-header">
        ...
        <div>
          ...
          <p className="text-body-sm text-secondary mt-1 max-w-[720px]">
            When you save a tool in Claude Code, Cursor, or a similar app, it can show up in My
            Toolkit on this site. Connect once per computer — you only type a short code; we never
            ask you to paste a long password.
          </p>
        </div>
      </div>
      <ol className="install-steps agent-link-steps" data-testid="agent-link-steps-guide">
        <li data-testid="agent-link-step-1">
          ...
          like ABCD-EFGH appears in the terminal.
        </li>
  ...
  if (!isAuthenticated) {
    return (
      ...
      >
        ...
        <Link
          href="/login?return_to=%2Fconnect%23agent-sync"
  ...
  return (
    ...
    >
      ...
      <div
        className={`agent-link-status ${linked ? "agent-link-status--linked" : "agent-link-status--pending"}`}
        role="status"
        aria-live="polite"
        data-testid={linked ? "agent-link-status-linked" : "agent-link-status-pending"}
      >
        <ShieldCheck size={18} aria-hidden />
        <span>{linked ? "Connected" : "Not connected yet"}</span>
      </div>
      {linked && (
        ...
        <fieldset className="agent-link-form mt-3 border-0 p-0 m-0">
          ...
          <form onSubmit={handleApprove}>
            ...
            <p id={hintId} className="text-body-sm text-secondary mt-2">
              8 characters, format ABCD-EFGH. Type exactly as shown in your terminal.
            </p>
          </form>
        </fieldset>
      </details>
