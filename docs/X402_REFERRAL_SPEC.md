...
## 2026-06-27 구현 결정 업데이트

- 이번 구현은 attribution/referral metadata + 로컬 이벤트 기록이다. OnchainAI는 facilitator 프록시, 커스터디, 사용자/제공자 자금 이동 코드를 만들지 않는다.
- x402 결제 검증은 신뢰 신호일 뿐 공개/MCP 노출 hard gate가 아니다. x402가 verify 안 되어도 기존 public quality gate를 통과하면 public/MCP 노출 가능하다.
- 결제 요청에 문서화되지 않은 `referrer`, `split` 필드를 invent하지 않는다. attribution은 `x402_builder_code` 같은 명시 메타 경로를 우선한다.
- `referral_model = 'split'`은 운영/비즈니스 합의 라벨이며, upstream provider가 명시 지원하지 않는 한 프로토콜 레벨 자동 split 보장이 아니다.
- `PUBLIC_TOOL_WHERE`와 public RLS에는 `payment_verified`, `x402_endpoint_verified`, `price_verified`를 추가하지 않는다. 미래에 숨김 정책을 바꾸려면 별도 operator decision이 필요하다.
...
### 작동 개념 (말로)
1. 디렉토리는 각 x402 도구에 대해 builder/attribution metadata(`x402_builder_code`, payout 주소, bps)를 함께 노출.
2. 에이전트/도구 제공자는 이 attribution metadata를 문서화된 방식으로 사용할 수 있다. 문서화되지 않은 결제 요청 필드는 invent하지 않는다.
3. 도구 제공자가 별도 합의나 upstream 지원 방식으로 정산 시, 합의된 비율만큼 디렉토리 지갑으로 분배.
4. 디렉토리는 클릭/설치 안내 횟수를 자체 기록(어트리뷰션)해서 분배 협상/대조 근거로 쓴다.
...
- `attribution_session`(익명 세션/유저, nullable), `created_at`,
...
### 공개/MCP 노출 정책
- 모든 도구는 기존 public quality gate를 따른다.
- x402/referral 도구도 `payment_verified AND x402_endpoint_verified AND price_verified`가 없어도 공개/MCP 노출 가능하다.
- 검증 플래그는 public 상세/MCP install guide/admin UI의 trust signal이다.
- 서버(`PUBLIC_TOOL_WHERE`)와 DB RLS 정책은 같은 public quality gate를 유지한다. 결제 검증 플래그를 visibility gate로 추가하지 말 것.
...
1. 마이그레이션 017: 컬럼·테이블·RLS sync. x402 검증 플래그는 hard gate가 아니라 trust signal.
...
6. 테스트: 검증 안 된 x402 도구도 public gate에서 배제되지 않음, 어트리뷰션 기록, sanitize(민감정보 비노출). 기존 `cargo test --features ssr` + clippy/fmt 통과.

## 9. 검증 주의 (실측 필요)

- x402 spec / facilitator API / split·referrer류 필드는 빠르게 진화 중. **구현 전 현재 x402 공식 스펙 실측 확인.** 문서화되지 않은 결제 요청 필드나 정산 방식은 만들지 말 것.
- USDC/EIP-3009/체인별 정산 주소 형식 확인.

---

### 한 줄 요약
A(레퍼럴, 커스터디 없음) 채택. 도구에 referral 메타+검증 플래그 추가 → x402 검증은 숨김 조건이 아니라 trust signal → 어트리뷰션 로그로 정산 대조 → 운영자 UI/공개 UX/투명고지. facilitator 커스터디는 범위 밖.
