...
- **Domain binding**: 메시지에 `onchainai.xyz` 도메인 포함 → 다른 사이트에서 서명 재사용 방지
...
**서명 메시지 예시**:
```
onchainai.xyz wants you to sign in with your Ethereum account:
0x1234...abcd

Sign in to OnchainAI to comment, upvote, and bookmark tools

URI: https://onchainai.xyz/auth/siwx
Version: 1
Chain ID: 1
Nonce: a3b1c9f2e7d4...
...
- **서명 전 확인**: 지갑에 표시되는 메시지에 도메인(onchainai.xyz) 명시 → 피싱 시 유저가 도메인 불일치 감지
...
- **Origin 헤더 검증**: POST/PUT/DELETE 요청 시 `Origin` 헤더가 `onchainai.xyz`인지 확인
