...
- 설치 명령: `/plugin marketplace add Coinyak/onchainai` → `/plugin install onchainai@onchainai`
...
- **목적**: 코딩 툴에서 명시적으로 저장한 툴이 웹 `/toolkit`·당일 agent-session blueprint에 반영.
- **플러그인 변경**: `plugin.json` `0.2.0`; `.mcp.json`에 `"Authorization": "Bearer ${ONCHAINAI_AGENT_TOKEN:-}"`; `find-tool.md`·`SKILL.md`에 `save_to_toolkit` / `link_required` 안내.
- **연결 UX**: 웹 `/connect#agent-sync` device flow 우선; PAT 복붙은 고급 경로.
- **MCP**: 인증 시 `save_to_toolkit`, `save_stack_to_blueprint`, `link_status` 추가; 무인증은 기존 5툴 유지.
- **상세 스펙**: [superpowers/specs/2026-07-04-agent-sync-toolkit-blueprint-spec.md](superpowers/specs/2026-07-04-agent-sync-toolkit-blueprint-spec.md)
