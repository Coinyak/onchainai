...
| 수취 지갑 | **오너 확정 필요(§6)** — OKX Provider 전용 지갑 분리 권장(`site_settings` payout 재사용 비권장) |
...
1. **OKX 수취 지갑 주소** — OKX Provider 전용 self-custody 지갑. **`site_settings` payout 재사용 비권장 — 분리 권장**(운영/OKX 수취 분리).
...
6. **수취 지갑 실측** — §3.3 `0x9626…`가 운영 DB `site_settings` 실제값인지 런타임 확인(git만으론 불가, 코드엔 테스트 픽스처로만).
