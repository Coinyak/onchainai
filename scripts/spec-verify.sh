...
# curl_has ID "desc" URL PATTERN
#   SKIP only when the connection fails (no network). A reachable endpoint that
#   404s or doesn't match the pattern is a real FAIL (no `-f`, so 404 != SKIP).
curl_has() { local id="$1" d="$2" url="$3" pat="$4"; want "$id" || return
  command -v curl >/dev/null || { skip "$id" "$d (no curl)"; return; }
  local out code body
  out="$(curl -sS -L --max-time 15 -w $'\n%{http_code}' "$url" 2>/dev/null)" || { skip "$id" "$d (무응답/네트워크)"; return; }
  [ -z "$out" ] && { skip "$id" "$d (무응답/네트워크)"; return; }
  code="${out##*$'\n'}"; body="${out%$'\n'*}"
  printf '%s' "$body" | grep -Eq -- "$pat" && ok "$id" "$d" || no "$id" "$d (http ${code:-?})"
}
...
# precise: a JSON *property key* "sort"/"cursor" (not the install-guide enum value "cursor")
present A4-params "search_tools inputSchema에 sort/cursor 파라미터" '"(sort|cursor|next_cursor)"[[:space:]]*:' src/server/mcp.rs
