...
use crate::server::rate_limit::{
    check_agent_device_start_limit, check_agent_token_mint_limit, check_user_rate_limit,
    client_ip_from_connect, UserRateLimitAction,
};
use crate::AppState;
...
async fn start_device(
    ...
    if let Err(limit) = check_agent_device_start_limit(&client_ip) {
        return Err(ApiError::TooManyRequests(limit.to_string()));
    }
