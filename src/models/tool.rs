...
/// Slim list/search hit — omits internal operator fields and dead `trust_score`.
#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]
...
/// Public catalog detail — operator/moderation fields omitted.
#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]
...
impl From<Tool> for PublicTool {
    fn from(tool: Tool) -> Self {
        ...
        Self {
            ...
            install_risk_level: tool.install_risk_level,
            install_risk_reasons: tool.install_risk_reasons,
            ...
            referral_enabled: tool.referral_enabled,
            referral_bps: tool.referral_bps,
            referral_model: tool.referral_model,
            payment_verified: tool.payment_verified,
            x402_endpoint_verified: tool.x402_endpoint_verified,
...
pub fn tools_to_public_summaries(tools: Vec<Tool>) -> Vec<PublicToolSummary> {
    tools.into_iter().map(PublicToolSummary::from).collect()
}
